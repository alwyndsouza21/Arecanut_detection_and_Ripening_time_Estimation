...
#Function to create Faster RCNN model for detectiion..
...
det_model.load_state_dict(torch.load("det_model.pth"))

cls_model,cls_transforms=create_classification_model()
cls_model.load_state_dict(torch.load("path_to_cls_model.pth"))


#defining  all the functios to be used later
#___________________________________________________________________________________________________________________________________________________
...
def predict_and_plot_bounding_boxes(model, image_path):
  ...
  keep = nms(torch.tensor(box_coordinates), torch.tensor(scores), iou_threshold=0.013)
...
#function to classiffy the cropped regions
...
#from going_modular_classification import classifi
def predict(image_dir):
  classes=["infloroscence","req_2.5M","req 1.5M","req 1M","req 20D","Ready!!"]
  pred_classes=[]
  cropped_tensors,boxes,scores=predict_and_plot_bounding_boxes(det_model,image_dir)
...
def plot_bboxes(image_dir, boxes, classes, scores=None, figsize=(10, 10)):
    ...
    for i, box in enumerate(boxes):
        ...
        if scores is not None:
            score = scores[i]
            ax.text(x1, y1 - 5, f"{class_name}|{score:.2f}", color='black', fontsize=16)
        else:
            ax.text(x1, y1 - 5, class_name, color='black', fontsize=16)
